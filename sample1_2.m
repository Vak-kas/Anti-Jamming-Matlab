...
%% Result Storage
NCT_all = zeros(length(etaList), T);
NCT_raw_all = zeros(length(etaList), T);

masterSeed = randi(100000);

%% Eta Comparison Experiment
for etaIdx = 1:length(etaList)
    ...
    [txNodes, rxNodes] = createNodes(N, area, power, @randomFHP, pairDistance, Tc_ms);
    ...
    NCT_raw_all(etaIdx, :) = NCT;
    NCT_all(etaIdx, :) = movmean(NCT, 50);

    fprintf("ETA %.1f | Mean NCT 900~1000 = %.4f\n", ...
        eta, mean(NCT(900:1000)));
...
plot(1:T, NCT_all(1, :), 'LineWidth', 2);
plot(1:T, NCT_all(2, :), 'LineWidth', 2);
plot(1:T, NCT_all(3, :), 'LineWidth', 2);
plot(1:T, NCT_all(4, :), 'LineWidth', 2);
plot(1:T, NCT_all(5, :), 'LineWidth', 2);

xlabel('Communication Time Slot');
ylabel('Normalized Communication Throughput');
title('NCT Comparison for Different Weight Coefficients');
...
for etaIdx = 1:length(etaList)
    fprintf("eta = %.1f | Mean 800~1000 = %.4f | Mean 900~1000 = %.4f\n", ...
        etaList(etaIdx), ...
        mean(NCT_raw_all(etaIdx, 800:1000)), ...
        mean(NCT_raw_all(etaIdx, 900:1000)));
end
